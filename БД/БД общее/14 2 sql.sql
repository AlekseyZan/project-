...
-- выбрать все лайки и пользователей 
select * from users as u
join likes as l on u.id = l.user_id;

-- узнать длительность всех лайканых видео пользователя 1
select duration from users as u
join likes as l on l.user_id = u.id AND u.username = 'user1'
join videos as v on v.id = l.video_id;

-- узнать все комментарии от пользователей с айди > 2
select * from comments as c
join users as u on u.id > 2 AND u.id = c.author_id;

-- дату загрузки и длительность видео от пользователя с айди 5
