...
JOIN users_to_chats where u.id = c.id ORDER BY description desc;
...
utc.enter_datetime
FROM users AS u, chats AS c,users_to_chats AS utc
JOIN users_to_chats WHERE c.id = u.id ORDER BY enter_datetime;

SELECT
c.title,
utc.enter_datetime
FROM users AS u, chats AS c, users_to_chats AS utc
JOIN users_to_chats WHERE u.id = c.owner_id;