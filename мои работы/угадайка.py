...
def play(word):
    world_complition = '_'*len(word)
    quessed = False
    quessed_letters = []
    quessed_words = []
    tries = 6
    print('Добро пожаловать в игру "Угадайка слов"')
    print(display_hangman(tries))
    print(world_complition)
    while True:
        word_input = input("Введите слово или букву   ").upper()
        if  not  word_input.isalpha():
            print('Неккоректный ввод. Необходимо ввести букву или слово')
            continue
        if word_input in quessed_words or word_input in quessed_letters:
            print('Уже было.Попробуйте что-нибудь другое')
        continue
        if len(word_input)>1:
            if word_input==word:
                print("Поздравляем!!! Вы угадали слово!!! УРА,ПОБЕДА!!!")
                break
            else:
                quessed_words.append(word_input)
                tries-=1
                print(f'Не угадали,осталось {tries} попыток')
                print(display_hangman(tries))
                print_word(word,quessed_letters)
                if tries==0:
                    print(f"К сожалению,вы не смогли угадать слово:{word}")
                    break
                continue
        if word_input in word:
            quessed_letters.append(word_input)
            for c in word:
                if c not in quessed_letters:
                    print("Угадали букву,продолжаем играть")
                    print_word(word, quessed_letters)
                    quessed = False
                    break
                quessed = True
            ...
            quessed_letters.append(word_input)
            tries-=1
            print(f'Опять мимо,осталось {tries} попыток')
            print(display_hangman(tries))
            print_word(word,quessed_letters)
            if tries==0:
                print(f"К сожалению,вы не смогли угадать слово:{word}")
